...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ASSUME1(size >= kPngHeaderSize);
  ScopedPngObject O;
  ASSUME1(png_sig_cmp(const_cast<uint8_t*>(data), 0, kPngHeaderSize) == 0);
  ...
  ASSUME1(png_get_IHDR(png_ptr, info_ptr, &width, &height,
        &bit_depth, &color_type, &interlace_type,
        &compression_type, &filter_type));

  ASSUME1(height * width <= 2000000); // To not be to slow
