...
POSSIBLE_FUZZING_ENGINE="libfuzzer afl afl_custom gcov coverage fsanitize_fuzzer hooks"
...
if [[ $FUZZING_ENGINE == "fsanitize_fuzzer" ]]; then
  ...
  AFL_CUSTOM_FLAGS="-O2 -fno-omit-frame-pointer -gline-tables-only -fsanitize=address"
...
elif [[ $FUZZING_ENGINE == "gcov" ]]; then
  GCOV_FLAGS="-O2 -g --coverage"
  export CFLAGS=${CFLAGS:-$GCOV_FLAGS}
  export CXXFLAGS=${CXXFLAGS:-$GCOV_FLAGS}
else
